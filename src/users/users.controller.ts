...
export class UsersController {
  ...
  @Get()
  async findAll(@Query('email') email?: string): Promise<ResponseData<UserResponseDto[]>> {
    let users = await this.usersService.findAll(email);
  ...
  async findOne(@Param('id') id: string,): Promise<ResponseData<UserResponseDto>> {
    ...
    return new ResponseData<UserResponseDto>(user, HttpStatus.SUCCESS, HttpMessage.SUCCESS);  // +id để ép kiểu number vì lấy ở Param là string
  }
